        ...
        return new Promise( function ( resolve, reject ) {
            ...
            if ( r.length == 100 ) {
                ...
                var data__insertmany = function ( db, callback ) {
                    ...
                    db.collection( 'redpostdetails1' ).insertMany( r, function ( err, docs ) {
                        assert.equal( err, null );
                        callback( docs );
                    } );
                };
                ...
                MongoClient.connect( url, function ( err, db ) {
                    ...
                    data__insertmany( db, function ( cb ) {
                        ...
                        _processStatus.DataResult = [];
                        _this.readData();
                    } );
                } );
                
            }
            else if ( t.idx == t.filesCount.length ) {
                
                var data__insertmany1 = function ( db, callback ) {
                ...
                MongoClient.connect( url, function ( err, db ) {
                    assert.equal( null, err );
                    data__insertmany1( db, function ( cb ) {
                        logger.log( 'info', 'insertDataOK! ---- 0' );
                        db.close( function () {
                            console.log( "dbClose" )
                        } );
                    } );
                ...
                var text     = fs.readFileSync( path + "/" + data_dir + "/" + t.idx + ".txt", 'utf-8' )
                ...
                if ( title == '迷路了吧' || text == 'undefined' ) {
                    ...
                    logger.log( 'error', "error__File : ", t.idx + ".txt" , " | " , r.length )
                    ++t.idx;
                    //++_processStatus.max_data_num;
                    _this.readData();
                }
                    ...
                    logger.log( 'info', id, regDate, '===>', t.idx , " | " , r.length );
