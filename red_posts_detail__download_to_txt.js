    ...
    _this.process_1 = function () {
        return new Promise( function ( resolve ) {
            ...
            var dataSelect = function ( db, callback ) {
                ...
                db.collection( 'redposts1' ).find( { p_detail : "N" }, { _id : 0,post_id :1 }).toArray( function ( err, docs ) {
    ...
    _this.process_1__1 = function ( data ) {
        ...
        return new Promise( function ( resolve ) {
            ...
            for ( var i = 0 ; i < data.length ; i++ ) {
                var fileFullpath = '../redCrwData/saveCompleteData/posts_detail/';
                var dirName      = new Date().toFormat( 'YYYYMMDDHH24MISS' ).substring( 0, 10 );
                _processStatus.save_dir_name = dirName;
                var fileType     = '.txt';
                var filename     = data[ i ].id + fileType;
    ...
    _this.process_2 = function ( data ) {
        ...
        return new Promise( function ( resolve ) {
            ...
            var reqHtmlRead = function () {
                if ( t.idx == t.len ) {
                    ...
                    try {
                        ...
                        request( t.url.post + data[ t.idx ].post_id, function ( err, res, body ) {
                            if ( err ) {
                                logger.log( 'err', err )
                            }
            ...
            var detailYN__update__Y = function ( id ) {
                ...
                //RedPosts.update( { id : updateId }, { 'detailYN' : 'Y' }, function ( err, docs ) {
                //    if ( err ) {
                //        console.log( err )
                //    }
...
console.log(updateId)
                var p_detail__update = function ( db, callback ) {
                    db.collection( 'redposts1' ).update( { post_id : updateId }, { $set : { p_detail : 'Y' } }, function ( err, docs ) {
                        assert.equal( err, null );
                        callback( docs );
                    } );
                };
            ...
            var resHtmlWrite = function ( html ) {
                ...
                var filename      = t.idx;
                var fileFullpath  = '../redCrwData/saveCompleteData/posts_detail/';
                var dirName       = _processStatus.save_dir_name;
                var fileType      = '.txt';
                ...
                //else {
                //    var i = new Date().toFormat( 'YYYYMMDDHH24MISS' ).substring( 0, 12 );
                //    fs.mkdir( fileFullpath + dirName + '_' + i, 0777 )
                //}
                ...
                fs.writeFile( fileResult, c, 'utf8', function ( err ) {
                  ...
                  //      console.log( data[ t.idx ].post_id )
                        detailYN__update__Y(data[ t.idx ].post_id)
                    }
                } );
    ...
    var folderName = new Date().toFormat( 'YYYYMMDDHH24' );
