    ...
    _this.process_1 = function () {
        return new Promise( function ( resolve ) {
            var dataSelect = function ( db, callback ) {
                ...
                db.collection( 'redposts' ).find( { detailYN : { $ne : 'Y' } }, {
                    id  : 1,
                    _id : 0
    ...
    _this.process_2 = function ( data ) {
        ...
        return new Promise( function ( resolve ) {
            ...
            var reqHtmlRead = function () {
                if ( t.idx == t.len ) {
                    ...
                    try {
                        ...
                        request( t.url.post + data[ t.idx ].id, function ( err, res, body ) {
                            if ( err ) {
                                logger.log( 'err', err )
                            }
                            
                            resHtmlWrite( data[ t.idx ].id, body )
            ...
            var detailYN__update__Y = function ( id ) {
                var updateId   = id;
                var dataSelect = function ( db, callback ) {
                    db.collection( 'redposts' ).update( { id : updateId }, { $set : { detailYN : 'Y' } }, function ( err, docs ) {
                        assert.equal( err, null );
                        callback( docs );
                    } );
                };
